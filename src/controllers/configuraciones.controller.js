...
exports.obtenerAutoOrdenDropi = catchAsync(async (req, res) => {
  const { id_configuracion } = req.body;
  const [row] = await db.query(
    'SELECT auto_crear_orden_dropi FROM configuraciones WHERE id = ? AND suspendido = 0 LIMIT 1',
    { replacements: [id_configuracion], type: db.QueryTypes.SELECT },
  );
  res.json({
    status: 'success',
    activo: Number(row?.auto_crear_orden_dropi) === 1,
  });
});

exports.actualizarAutoOrdenDropi = catchAsync(async (req, res) => {
  const { id_configuracion, activo } = req.body;
  const val = activo === true || activo === 1 || activo === '1' ? 1 : 0;
  await db.query(
    'UPDATE configuraciones SET auto_crear_orden_dropi = ?, updated_at = NOW() WHERE id = ?',
