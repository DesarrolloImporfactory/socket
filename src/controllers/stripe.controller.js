...
const assertEnv = (name, value, extraCheck = null) => {
  const errors = [];
  const val = value;

  if (isBlank(val)) {
    errors.push(`Falta o está vacío: ${name}`);
    return { ok: false, errors };
  }

  if (typeof extraCheck === 'function') {
    const msg = extraCheck(val);
...
const mustBeHttpUrl = (v) => {
  const s = String(v).trim();
  if (!/^https?:\/\/.+/i.test(s))
    return `URL inválida (debe iniciar con http:// o https://). Valor: ${s}`;
  return null;
};

const mustLookLikeStripeKey = (v, expectLive) => {
  const s = String(v).trim();
  if (!/^sk_(test|live)_/i.test(s))
    return `Formato inválido de Stripe key (debe iniciar con sk_test_ o sk_live_).`;
  const isLive = /^sk_live_/i.test(s);
  if (expectLive && !isLive)
    return `En producción debería ser sk_live_... pero parece sk_test_.`;
  if (!expectLive && isLive)
    return `En no-producción debería ser sk_test_... pero parece sk_live_.`;
  return null;
};

const mustLookLikeCoupon = (v) => {
  const s = String(v).trim();
  // Stripe coupons suelen ser tipo "coup_..."
  // (Si usted usa IDs antiguos personalizados, puede ajustar esto)
  if (!/^coup_/i.test(s))
    return `Cupón no luce como coupon id (esperado coup_...). Valor: ${s}`;
  return null;
};

const mustLookLikePrice = (v) => {
  const s = String(v).trim();
  if (!/^price_/i.test(s))
    return `id_price inválido (esperado price_...). Valor: ${s}`;
  return null;
};

const logStripeEnvHealth = ({
  isProd,
  STRIPE_SECRET,
  FRONT_SUCCESS_URL,
  FRONT_CANCEL_URL,
  ...
  checks.push(assertEnv('NODE_ENV', process.env.NODE_ENV));
  checks.push(
    assertEnv('STRIPE_SECRET (seleccionada)', STRIPE_SECRET, (v) =>
      mustLookLikeStripeKey(v, isProd),
    ),
  );

  checks.push(
    assertEnv(
      'FRONT_SUCCESS_URL (seleccionada)',
      FRONT_SUCCESS_URL,
      mustBeHttpUrl,
    ),
  );
  checks.push(
    assertEnv(
      'FRONT_CANCEL_URL (seleccionada)',
      FRONT_CANCEL_URL,
      mustBeHttpUrl,
    ),
  );

  // Cupones: solo valide si existen (porque puede que no aplique a todos)
  if (!isBlank(COUPON_PLAN2))
    checks.push(
      assertEnv(
        'COUPON_PLAN2 (seleccionada)',
        COUPON_PLAN2,
        mustLookLikeCoupon,
      ),
    );
  if (!isBlank(COUPON_PLAN3))
    checks.push(
      assertEnv(
        'COUPON_PLAN3 (seleccionada)',
        COUPON_PLAN3,
        mustLookLikeCoupon,
      ),
    );
  if (!isBlank(COUPON_PLAN4))
    checks.push(
      assertEnv(
        'COUPON_PLAN4 (seleccionada)',
        COUPON_PLAN4,
        mustLookLikeCoupon,
      ),
    );
  ...
  // Plan conexión id
  if (
    !Number.isFinite(Number(STRIPE_PLAN_CONEXION_ID)) ||
    Number(STRIPE_PLAN_CONEXION_ID) <= 0
  ) {
    checks.push({
      ok: false,
      errors: [`STRIPE_PLAN_CONEXION_ID inválido: ${STRIPE_PLAN_CONEXION_ID}`],
    });
  }
  ...
  console.log('[stripe-env] env:', {
    ...
    STRIPE_SECRET: mask(STRIPE_SECRET),
    FRONT_SUCCESS_URL,
    FRONT_CANCEL_URL,
    COUPON_PLAN2: isBlank(COUPON_PLAN2) ? '(empty)' : mask(COUPON_PLAN2),
    COUPON_PLAN3: isBlank(COUPON_PLAN3) ? '(empty)' : mask(COUPON_PLAN3),
    COUPON_PLAN4: isBlank(COUPON_PLAN4) ? '(empty)' : mask(COUPON_PLAN4),
    STRIPE_PLAN_CONEXION_ID,
  });

  if (!ok) {
    console.error('[stripe-env] FALTAN / INVALIDAS:', errors);
  } else {
...
exports.crearSesionPago = catchAsync(async (req, res, next) => {
  ...
  // ===== DEBUG ENV (producción) =====
  const envHealth = logStripeEnvHealth({
    isProd,
    STRIPE_SECRET,
    FRONT_SUCCESS_URL,
    FRONT_CANCEL_URL,
  ...
  if (!envHealth.ok) {
    // Corte temprano con error claro
    return next(
      new AppError(
        `Stripe ENV incompleto/incorrecto: ${envHealth.errors.join(' | ')}`,
        500,
      ),
    );
  }

  // Validación extra: id_price del plan
  const priceCheck = assertEnv(
    'plan.id_price',
    plan.id_price,
    mustLookLikePrice,
  );
  if (!priceCheck.ok) {
    return next(
      new AppError(
        `Plan sin id_price válido: ${priceCheck.errors.join(' | ')}`,
        400,
      ),
    );
  }
