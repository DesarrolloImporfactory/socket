...
async function convertVideoForWhatsApp(fileBuffer, originalName) {
  ...
  try {
    ...
    // formula: (15MB * 8bits) / duration - audio_bitrate
    const MAX_BYTES = 15 * 1024 * 1024;
    const AUDIO_KBPS = 96;
    const totalKbps = Math.floor((MAX_BYTES * 8) / duration / 1000);
    const videoKbps = Math.max(100, totalKbps - AUDIO_KBPS); // mínimo 100k
    ...
    console.log(`[VIDEO_CONVERT] Tamaño final: ${(stat.size / (1024 * 1024)).toFixed(2)} MB`);
